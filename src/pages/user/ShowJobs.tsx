...
const ShowJobs: React.FC = () => {
  ...
  const handleSearch = (query: string) => {
    ...
    const filtered: IJob[] = jobs.filter((job) =>
        ...
        <div className="text-black">
          ...
          <div className="flex flex-wrap gap-5 mx-5">
            {filteredJobs.map((job) => (
              <div
                key={job._id}
                className="w-full md:w-60 h-auto bg-gradient-to-r  from-yellow-100 rounded-md to-white my-5 p-3"
                onClick={() => {
                  handleJobDetials(job._id);
                }}
              >
                <div className="text-start font-semibold">
                  ...
                  <div className="flex gap-4">
                    <p
                      className={`border-green-600 border-2 px-1 font-sans text-xs w-20 rounded-md ${
                        job.jobType === "Part Time" ||
                        job.jobType === "Full time"
                          ? "bg-green-300 font-semibold text-green-800"
                          : "bg-red-300 font-semibold text-red-800"
                      }`}
                    >
                      {job.jobType}
                    </p>
